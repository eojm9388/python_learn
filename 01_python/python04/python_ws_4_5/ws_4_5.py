...
def create_user(user_data_dummy):
    user_list = []
    invalid_data_count = 0
    for i in range(len(user_data_dummy)):
        test, invalid_data = is_validation(user_data_dummy[i])
        
        if test:
            user_list.append(user_data_dummy[i])
        else:
            invalid_data_count += 1
            if invalid_data != 'blocked':
                for data in invalid_data:
                    user_data[i][data] = None
                user_list.append(user_data_dummy[i])
    print(f'잘못된 데이터로 구성된 유저의  수는 {invalid_data_count}입니다.')
...
def is_validation(user):
    ...
    if blood_group not in blood_types:
        test = False
        test_list.append('blood_group')

    elif company in black_list:
        return (False, 'blocked')
    
    elif '@' not in mail:
        test = False
        test_list.append('mail')
        ...
        test = False
        test_list.append(website)

    if not test:
        return (False, test_list)
    else:
        return (True, None)
